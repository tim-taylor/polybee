...
namespace lemon {
  ...
  private:
    ...
    ArrayMap(const ArrayMap& copy) : Parent() {
      ...
      values = AllocTraits::allocate(allocator, capacity);
      Notifier* nf = Parent::notifier();
      Item it;
      for (nf->first(it); it != INVALID; nf->next(it)) {
        int id = nf->id(it);;
  ...
  protected:
    ...
    virtual void add(const Key& key) {
      ...
      if (id >= capacity) {
        ...
        Value* new_values = AllocTraits::allocate(allocator, new_capacity);
        Item it;
        for (nf->first(it); it != INVALID; nf->next(it)) {
          int jd = nf->id(it);;
          if (id != jd) {
        ...
        if (capacity != 0) AllocTraits::deallocate(allocator, values, capacity);
        values = new_values;
    ...
    virtual void clear() {
      ...
      if (capacity != 0) {
        ...
        AllocTraits::deallocate(allocator, values, capacity);
        capacity = 0;
      }
    }
